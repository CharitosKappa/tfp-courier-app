...
Beginner summary:
- Upload courier invoices (ZIP or normal files) + Odoo exports.
- The app parses rows, dedupes, stores in DB (Neon Postgres via DATABASE_URL / st.secrets).
- Shows progress + ETA while processing, and lets you export everything to Excel.

Security:
- DB URL is never printed.
"""
...
APP_VERSION = "v7.1 (fixed nonlocal, progress+ETA, zip/non-zip, faster bulk writes)"
...
LEGACY_7DIGIT_RE = re.compile(r"^\d{7}$")  # old e-shop ref => exclude from ALL calculations
TRACKING_NUM_RE = re.compile(r"\b\d{9,20}\b")  # generic tracking-like

# Optional BoxNow line heuristic (best-effort; PDFs vary)
BOXNOW_LINE_RE = re.compile(r"\b(?P<tracking>\d{9,20})\s+(?P<date>\d{1,2}/\d{1,2}/\d{4})\s+.*?(?P<amt>\d+,\d{2})\b")
...
def coerce_ref(x) -> str:
    """Make references stable (e.g. 1234567.0 -> 1234567)."""
    if x is None:
        return ""
...
def upsert_many(engine: Engine, table: str, rows: List[dict], pk: str) -> Tuple[int, int]:
    """
    Bulk upsert via SQLAlchemy executemany.

    This is the main speed-up vs row-by-row inserts.
    """
    if not rows:
        return 0, 0
...
def parse_odoo_pickings_xlsx(b: bytes) -> pd.DataFrame:
    ...
    c_done = safe_col(df, ["Date of Transfer", "Date Done", "Done Date"])
    ...
    c_track = safe_col(df, ["Tracking Reference", "Tracking", "Tracking number"])
...
def parse_acs_gr_xlsx(b: bytes) -> pd.DataFrame:
    ...
    c_date = safe_col(df, ["Ημ/νια παράδοσης", "Ημ/νια παραλαβής"])
    c_amount = safe_col(df, ["Καθαρή Αξία Τιμολογίου"])
    c_ref1 = safe_col(df, ["Σχετικό 1"])
    c_ref2 = safe_col(df, ["Σχετικό 2"])

    if not c_amount:
        raise KeyError("ACS GR: cannot find 'Καθαρή Αξία Τιμολογίου' column.")
    ...
    # amounts: "2,3100"
    out["amount_net"] = (
        out["amount_net"].astype(str).str.replace(".", "", regex=False).str.replace(",", ".", regex=False)
    )
...
def parse_courier_center_xls(b: bytes) -> pd.DataFrame:
    # Requires xlrd (in requirements.txt)
...
def parse_boxnow_pdf(b: bytes) -> pd.DataFrame:
    t = extract_pdf_text(b, max_pages=50)

    inv_no = None
    m = re.search(r"(ΤΠΥ[-\s]?\d+)", t, flags=re.IGNORECASE)
    if m:
...
def main():
    ...
    st.write("Upload ZIP (bulk) or normal files. The app handles both. For Odoo exports, use the plain .xlsx files.")
    ...
    if st.button("Process now", type="primary"):
        ...
        # Build worklist (courier only; Odoo is processed after)
        work = []
        ...
        # courier: 4 phases per file + odoo: 3 phases per file (max 4 files)
        ...
        # Process courier files
        for slot, fname, b, container, ftype, parser in work:
            display = f"{container} → {fname}"
            try:
                done += 1
                progress_update(pb, status_box, metrics_box, done, total_steps, "Hashing + dedupe", display, ok_rows, skipped_files, failed, start_ts)
                ...
                progress_update(pb, status_box, metrics_box, done, total_steps, "Parsing", display, ok_rows, skipped_files, failed, start_ts)
                df = parser(b)

                done += 1
                progress_update(pb, status_box, metrics_box, done, total_steps, "Normalizing", display, ok_rows, skipped_files, failed, start_ts)
                ...
                progress_update(pb, status_box, metrics_box, done, total_steps, "DB write (bulk)", display, ok_rows, skipped_files, failed, start_ts)
        ...
        # Process Odoo (helper returns updated counters)
        def process_odoo(uploaded_file, ftype: str, parse_fn, table: str, pk: str, label: str,
                         done: int, ok_rows: int, skipped_files: int, failed: int, errors: list):
            if uploaded_file is None:
                return done, ok_rows, skipped_files, failed, errors

            try:
                done += 1
                progress_update(pb, status_box, metrics_box, done, total_steps, "Odoo hashing + dedupe", label, ok_rows, skipped_files, failed, start_ts)
                ...
                progress_update(pb, status_box, metrics_box, done, total_steps, "Odoo parsing", label, ok_rows, skipped_files, failed, start_ts)
                df = parse_fn(b)
                df["source_file_hash"] = h
                rows = df.to_dict("records")

                done += 1
                progress_update(pb, status_box, metrics_box, done, total_steps, "Odoo DB write (bulk)", label, ok_rows, skipped_files, failed, start_ts)
                for i in range(0, len(rows), chunk_size):
                    chunk = rows[i : i + chunk_size]
        ...
        done, ok_rows, skipped_files, failed, errors = process_odoo(
            up_pick_orders, "odoo_pickings_orders", parse_odoo_pickings_xlsx,
            "odoo_pickings", "picking_name", "Odoo Pickings Orders",
            done, ok_rows, skipped_files, failed, errors
        )

        done, ok_rows, skipped_files, failed, errors = process_odoo(
            up_pick_returns, "odoo_pickings_returns", parse_odoo_pickings_xlsx,
            "odoo_returns", "picking_name", "Odoo Pickings Returns",
            done, ok_rows, skipped_files, failed, errors
        )

        done, ok_rows, skipped_files, failed, errors = process_odoo(
            up_sales, "odoo_sales_orders", parse_odoo_sales_orders_xlsx,
            "odoo_sales_courier_fees", "order_ref", "Odoo Sales Orders",
            done, ok_rows, skipped_files, failed, errors
        )

        done, ok_rows, skipped_files, failed, errors = process_odoo(
            up_rma, "odoo_rma", parse_odoo_rma_xlsx,
            "odoo_rma_fees", "rma_ref", "Odoo RMA",
            done, ok_rows, skipped_files, failed, errors
        )

        pb.progress(100, text="Completed")
        elapsed = int(dt.datetime.utcnow().timestamp() - start_ts)
        st.success(f"Completed in {elapsed}s | OK rows={ok_rows} | Skipped files={skipped_files} | Failed={failed}")
